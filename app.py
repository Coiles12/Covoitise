...
def sync_users_db():
    ...
    if not User.query.filter_by(pseudo='Gustave').first():
        ...
        db.session.add(User(pseudo='Gustave', password=hashed_admin_pw, credits=9999, first_login=True))
...
def cancel_ride(ride_id):
    ...
    try:
        ...
        if heures_restantes > 48:
            ...
            remboursement = int(cout * 0.75)
            msg = "Remboursement partiel (75%)"
...
def validate_scan():
    if 'user_id' not in session or session.get('pseudo') != 'Gustave':
